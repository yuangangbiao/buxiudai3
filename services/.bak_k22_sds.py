...
class ScheduleDispatchService:
    ...
    def publish_schedule(cls, order_no: str, order: Dict[str, Any],
        ...
        try:
            ...
            # [K22 BUG1 修复 2026-06-16] 第二次去重：查 system.db.tbl_documents.process_tasks JSON
            # 原始去重查的是 schedule_queue (MySQL),但脏数据实际存的是 process_tasks JSON 数组
            # 两者数据源不一致 → schedule_queue 是空/被清后,publish_schedule 误判"未发过",重新发布 → 47 条重复
            # 修复策略: 同时检查 process_tasks JSON 数组,出现过的 order_no 跳过
            try:
                from pathlib import Path as _Path
                _system_db = _Path(__file__).resolve().parent.parent / 'data' / 'system.db'
                if _system_db.exists():
                    import sqlite3 as _sqlite
                    import json as _json
                    _sc = _sqlite.connect(str(_system_db))
                    _sc.row_factory = _sqlite.Row
                    _row = _sc.execute(
                        "SELECT doc_data FROM tbl_documents WHERE id='dispatch_center_data'"
                    ).fetchone()
                    if _row:
                        _doc = _json.loads(_row['doc_data'])
                        _existing_orders = {
                            t.get('order_no') for t in _doc.get('process_tasks', [])
                            if t.get('order_no')
                        }
                        if order_no in _existing_orders:
                            _sc.close()
                            log_ui("排产调度", "重复提交(JSON)", f"工单={order_no} 在 process_tasks 已存在，跳过")
                            return {
                                'success': True,
                                'message': f'工单 {order_no} 已在 process_tasks 中，跳过重复',
                                'duplicate': True,
                            }
                    _sc.close()
            except Exception as _dedup_e:
                logger.warning(
                    f"[排产调度] process_tasks 去重校验异常(非致命): {_dedup_e}",
