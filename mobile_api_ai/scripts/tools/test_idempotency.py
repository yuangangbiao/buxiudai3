...
log("=== 测试幂等性（order_no检查）===")

for i in range(3):
    r = requests.post('http://localhost:5002/api/schedule/publish',
        json={'order_no': 'WO-IDEM-002', 'order_no': f'ORD-IDEM-002-{i}',
              'product_name': '幂等测试', 'quantity': 10, 'unit': '件',
...
c = conn.cursor()
c.execute("SELECT id, order_no, order_no, created_at FROM process_records WHERE order_no='WO-IDEM-002'")
rows = c.fetchall()
log(f"数据库 WO-IDEM-002: {len(rows)} 条 (应为1)")
conn.close()
