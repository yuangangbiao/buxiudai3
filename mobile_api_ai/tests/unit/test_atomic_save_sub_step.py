...
验证目标:
1. 正常情况: 3 键去重 + operator 追加 + process_sub_steps 累加
2. 原子性: process_sub_steps 累加失败时, process_sub_steps 也回滚
...
def _cleanup(c, cur):
    ...
    # process_sub_steps 累加测试, 不删 (无关)
    c.commit()


def test_atomic_normal_dispatch():
    """正常派工 1 个工人, 应写入 1 行 + process_sub_steps 累加."""
    ...
    cur = c.cursor()
    _cleanup(c, cur)
    # 1) 确保有 process_sub_steps 行 (atomic_test 假定已存在, 否则 UPDATE 不影响行数)
    cur.execute(
        "INSERT IGNORE INTO process_sub_steps (related_order, related_process, completed_qty) "
        "VALUES (%s, %s, 0)",
        (TEST_ORDER, TEST_STEP))
    c.commit()
    # 2) 取累加前 completed_qty
    cur.execute(
        "SELECT completed_qty FROM process_sub_steps WHERE related_order=%s AND related_process=%s",
        (TEST_ORDER, TEST_STEP))
    row = cur.fetchone()
    ...
    assert after - before == 50, f"process_sub_steps 累加错误: {before} -> {after}"
    print("  [PASS] 正常派工: 1 行, operator=工人A, process_sub_steps +50")


def test_atomic_multi_operator_append():
    """多人派工, 合并为 1 行, operator 追加, process_sub_steps 累加 N 倍."""
    from storage.mysql_storage import MySQLStorage
    s = MySQLStorage()
    s.connect()
    c = _conn()
    ...
    assert after - before == 65, f"process_sub_steps 累加错误: {before} -> {after}"
    print(f"  [PASS] 多人派工: 1 行, operator={op}, process_sub_steps +65")


def test_atomic_rollback_on_pkg_failure():
    """process_sub_steps 累加失败时, process_sub_steps 也应回滚 (原子性)."""
    from storage.mysql_storage import MySQLStorage
    s = MySQLStorage()
    s.connect()
    c = _conn()
    ...
    raised = False
    try:
        # 故意让 related_process 字段超长触发 data 错误 (process_sub_steps.process 列存在, 但
        ...
        s.save_process_sub_step_with_pkg_update(
            ...
            pkg_process=TEST_STEP + '_nonexistent_xxx',  # 故意让 process_sub_steps UPDATE 不命中
            qty_delta=10)
    except Exception:
        raised = True
    ...
    _cleanup(c, cur)
    cur.close(); c.close()
    # 注: process_sub_steps UPDATE 即便未命中行, 也不报错 (UPDATE 无匹配是合法的).
...
def test_atomic_invalid_qty_rolls_back():
    ...
    cur = c.cursor()
    _cleanup(c, cur)
    # 注入: process_sub_steps.related_process 字段超长触发 Data too long for column
