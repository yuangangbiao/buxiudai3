...
def page_sync(src, tgt, sf, page_size=200, max_iter=20):
    last_time = datetime(1900, 1, 1, 0, 0, 0)
    total = 0
    for i in range(max_iter):
        try:
            n = _sync_table(src, tgt, sf, page_size, last_time)
        except Exception as e:
            print(f'  iter {i+1}: 异常 {e}')
            break
        if n == 0:
...
print('=== 翻页同步 orders (1900-01-01 起点) ===')
...
rc = cur.fetchone()['c']
conn.close()
print(f'container_center: orders={oc}/364  sub_steps={sc}  records={rc}')
