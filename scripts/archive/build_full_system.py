...
print("=" * 70)
print("  打包全功能版不锈钢网带跟单系统")
print("=" * 70)
print()

# 清理旧的临时目录
temp_dir = os.path.join(BASE_DIR, "temp_full_exe")
build_dir = os.path.join(BASE_DIR, "temp_build_full")
...
if result.returncode == 0:
    ...
    if os.path.exists(exe_path):
        ...
        # 复制其他必要文件
        if os.path.exists(os.path.join(BASE_DIR, "constants.py")):
            shutil.copy2(os.path.join(BASE_DIR, "constants.py"), os.path.join(target_dir, "constants.py"))
        
        # 验证
        if os.path.exists(target_path):
            print(f"\n[OK] 完整性检查通过！")
            print(f"    全功能版EXE: {os.path.getsize(target_path) / (1024 * 1024):.2f} MB")
