...
def generate_license_key_from_fingerprint(fingerprint: str, customer_name: str = "", valid_days: int = 0) -> str:
    """基于指纹生成许可证密钥"""
    if valid_days > 0:
...
class LicenseActivationGUI:
            ...
            for widget_key in self.widgets:
                ...
                if isinstance(current_font, tuple):
                    ...
                    try:
                        ...
                        if hasattr(font_obj, 'actual'):
                            font_info = font_obj.actual()
                            family = font_info.get('family', 'Microsoft YaHei')
                            weight = font_info.get('weight', 'normal')
                            widget.config(font=(family, new_size, weight))
                    except Exception as e:
                        import logging
                        logging.getLogger(__name__).warning(f"字体调整失败: {e}")
        except Exception as e:
            import logging
            logging.getLogger(__name__).warning(f"UI初始化字体调整失败: {e}")
    ...
    def _setup_ui(self):
        ...
        self.font_sizes["title_label"] = 14
        ...
        self.font_sizes["subtitle_label"] = 8
        ...
        status_header = tk.Label(status_card, text="📊 当前状态",
                            ...
                            bg="#16213e", fg="#00d4ff")
        status_header.pack(anchor="w", padx=15, pady=(8, 5))
        self.widgets["status_header"] = status_header
        self.font_sizes["status_header"] = 10

        self.status_label = tk.Label(status_card, text="检查中...",
                                    font=("Microsoft YaHei", 9),
                                    bg="#16213e", fg="#888888", wraplength=360, justify="left")
        self.status_label.pack(anchor="w", padx=15, pady=(0, 8))
        self.widgets["status_label"] = self.status_label
        self.font_sizes["status_label"] = 9

        fingerprint_card = tk.Frame(card_frame, bg="#16213e")
        fingerprint_card.pack(fill=tk.X, pady=(0, 10))

        fp_header_frame = tk.Frame(fingerprint_card, bg="#16213e")
        fp_header_frame.pack(fill=tk.X, padx=15, pady=(8, 5))

        fp_header = tk.Label(fp_header_frame, text="🖥️ 机器指纹",
                           font=("Microsoft YaHei", 10, "bold"),
                           bg="#16213e", fg="#00d4ff")
        fp_header.pack(side=tk.LEFT)
        self.widgets["fp_header"] = fp_header
        self.font_sizes["fp_header"] = 10

        copy_btn = tk.Button(fp_header_frame, text="📋 复制",
                           font=("Microsoft YaHei", 8),
                           bg="#0f3460", fg="#00d4ff",
        ...
        self.font_sizes["copy_btn"] = 8

        fp_inner = tk.Frame(fingerprint_card, bg="#16213e")
        fp_inner.pack(fill=tk.X, padx=15, pady=(0, 8))

        self.fp_label = tk.Label(fp_inner, text="获取中...",
                                font=("Consolas", 10),
                                bg="#0a0a1a", fg="#00d4ff",
                                padx=10, pady=8, wraplength=350, justify="center")
        self.fp_label.pack(fill=tk.X)
        self.widgets["fp_label"] = self.fp_label
        self.font_sizes["fp_label"] = 10
        ...
        input_header = tk.Label(input_card, text="🔐 激活信息",
                              font=("Microsoft YaHei", 10, "bold"),
                              bg="#16213e", fg="#00d4ff")
        input_header.pack(anchor="w", padx=15, pady=(8, 5))
        self.widgets["input_header"] = input_header
        self.font_sizes["input_header"] = 10

        input_inner = tk.Frame(input_card, bg="#16213e")
        input_inner.pack(fill=tk.X, padx=15, pady=(0, 8))
        ...
        self.font_sizes["license_key_label"] = 9

        self.license_var = tk.StringVar()
        license_entry = ttk.Entry(input_inner, textvariable=self.license_var, width=28, font=("Consolas", 9))
        license_entry.grid(row=0, column=1, pady=5, padx=(8, 0), sticky="w")
        license_entry.focus()
        self.widgets["license_entry"] = license_entry
        self.font_sizes["license_entry"] = 9

        customer_label = tk.Label(input_inner, text="客户名称:",
                            font=("Microsoft YaHei", 9),
                            bg="#16213e", fg="#e0e0e0", width=10, anchor="w")
        customer_label.grid(row=1, column=0, pady=5, sticky="w")
        self.widgets["customer_label"] = customer_label
        self.font_sizes["customer_label"] = 9

        self.customer_var = tk.StringVar()
        customer_entry = ttk.Entry(input_inner, textvariable=self.customer_var, width=28, font=("Microsoft YaHei", 9))
        customer_entry.grid(row=1, column=1, pady=5, padx=(8, 0), sticky="w")
        self.widgets["customer_entry"] = customer_entry
        self.font_sizes["customer_entry"] = 9

        format_label = tk.Label(input_inner, text="格式: YGB-XXXX-XXXX-XXXX-XXXX-C",
                              font=("Microsoft YaHei", 7),
                              bg="#16213e", fg="#666666")
        format_label.grid(row=2, column=1, padx=(8, 0), sticky="w")
        self.widgets["format_label"] = format_label
        self.font_sizes["format_label"] = 7
        ...
        self.activate_btn.pack(side=tk.LEFT, padx=(0, 8))
        self.widgets["activate_btn"] = self.activate_btn
        self.font_sizes["activate_btn"] = 10
        ...
        self.deactivate_btn.pack(side=tk.LEFT, padx=(0, 8))
        self.widgets["deactivate_btn"] = self.deactivate_btn
        self.font_sizes["deactivate_btn"] = 10
    ...
    def _load_status(self):
        try:
            ...
            fp_short = fingerprint[:8].upper()
            self.fp_label.config(text=fingerprint.upper())
    ...
    def _copy_fingerprint(self):
        try:
            fingerprint = self.fp_label.cget("text")
            if fingerprint and fingerprint != "获取中...":
                self.root.clipboard_clear()
                self.root.clipboard_append(fingerprint)
                messagebox.showinfo("复制成功", f"机器指纹已复制到剪贴板:\n\n{fingerprint}")
            else:
                messagebox.showwarning("复制失败", "机器指纹尚未加载完成")
        except Exception as e:
...
class FingerprintUnlockGUI:
    ...
    def _setup_ui(self):
        ...
        left_scroll_frame = tk.Frame(left_canvas, bg="#1a1a2e", width=700)
        ...
        right_frame = tk.Frame(main_container, bg="#16213e", width=380)
    ...
    def _setup_left_panel(self, parent):
        header_frame = tk.Frame(parent, bg="#16213e", padx=20, pady=15)
        header_frame.pack(fill=tk.X)
        ...
        tk.Label(title_frame, text="生成唯一机器标识，用于软件授权",
                font=("Microsoft YaHei", 10),
                bg="#16213e", fg="#4a7fa5").pack(anchor="w")

        card_frame = tk.Frame(parent, bg="#1a1a2e", padx=20, pady=10)
        card_frame.pack(fill=tk.BOTH, expand=True)

        fingerprint_card = tk.Frame(card_frame, bg="#16213e", bd=0, relief=tk.FLAT)
        fingerprint_card.pack(fill=tk.X, pady=(0, 15))

        tk.Label(fingerprint_card, text="📋 机器指纹",
                font=("Microsoft YaHei", 14, "bold"),
                bg="#16213e", fg="#00d4ff").pack(anchor="w", padx=20, pady=(15, 10))

        self.fp_label = tk.Label(fingerprint_card, text="正在获取...",
                                 font=("Consolas", 11),
                                 bg="#16213e", fg="#00ff88", wraplength=640, justify="left")
        self.fp_label.pack(anchor="w", padx=20, pady=(0, 15))

        short_card = tk.Frame(card_frame, bg="#16213e", bd=0, relief=tk.FLAT)
        short_card.pack(fill=tk.X, pady=(0, 15))

        tk.Label(short_card, text="🔑 短指纹（8位）",
                font=("Microsoft YaHei", 14, "bold"),
                bg="#16213e", fg="#00d4ff").pack(anchor="w", padx=20, pady=(15, 10))
        ...
        self.short_fp_label = tk.Label(short_card, text="正在获取...",
                                       ...
                                       bg="#16213e", fg="#ffcc00")
        self.short_fp_label.pack(anchor="w", padx=20, pady=(0, 15))

        activation_status_card = tk.Frame(card_frame, bg="#16213e", bd=0, relief=tk.FLAT)
        activation_status_card.pack(fill=tk.X, pady=(0, 15))

        tk.Label(activation_status_card, text="📊 许可证状态",
                font=("Microsoft YaHei", 14, "bold"),
                bg="#16213e", fg="#00d4ff").pack(anchor="w", padx=20, pady=(15, 10))

        self.activation_label = tk.Label(activation_status_card, text="检查中...",
                                         font=("Microsoft YaHei", 11),
                                         bg="#16213e", fg="#888888", wraplength=640, justify="left")
        self.activation_label.pack(anchor="w", padx=20, pady=(0, 15))

        hardware_card = tk.Frame(card_frame, bg="#16213e", bd=0, relief=tk.FLAT)
        hardware_card.pack(fill=tk.X, pady=(0, 15))

        tk.Label(hardware_card, text="🖥️ 硬件信息",
                font=("Microsoft YaHei", 14, "bold"),
                bg="#16213e", fg="#00d4ff").pack(anchor="w", padx=20, pady=(15, 10))
        ...
        for i, (key, label) in enumerate(info_items):
            ...
            item_frame.grid(row=row, column=col, padx=5, pady=8, sticky="w")

            tk.Label(item_frame, text=f"{label}:",
                    font=("Microsoft YaHei", 10),
                    bg="#16213e", fg="#888888", width=14, anchor="w").grid(row=0, column=0, sticky="w")
        ...
        activate_btn_frame.pack(fill=tk.X, padx=20, pady=(0, 15))
        ...
        self.activate_btn.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 8))
        ...
        self.gen_key_result = tk.Label(button_card, text="",
                                      font=("Consolas", 10),
                                      bg="#16213e", fg="#00ff88")
        self.gen_key_result.pack(anchor="w", padx=20, pady=(5, 0))
    ...
    def _setup_right_panel(self, parent):
        tk.Label(parent, text="📖 使用说明",
                font=("Microsoft YaHei", 16, "bold"),
                bg="#16213e", fg="#00d4ff").pack(anchor="w", padx=20, pady=(20, 15))
...
(最后一位为校验位)"""
        ...
        tk.Label(parent, text=guide_text,
                font=("Microsoft YaHei", 10),
                bg="#16213e", fg="#cccccc", wraplength=340,
                justify="left", anchor="nw").pack(fill=tk.BOTH, expand=True, padx=20, pady=(0, 20))
